//! Strategy Layer v2 - Baseline-First Validation Runner
//!
//! PRIMARY: ON > Baseline (not just ON > OFF)
//! Target: 2/3 games ON > Baseline

use agl_mwe::prior_channel::{
    Marker, MarkerScheduler,
    strategy_layer_v2::{
        GamePolicyV2, GameType,
        game_policies_v2::coop_probability_v2,
        validation_v2::{BatchValidation, GameValidation, ConditionResult},
    },
...
impl Agent {
    ...
    fn act(&mut self, markers: &[Marker], round: usize) -> Action {
        match self.condition {
            ...
            Condition::Off => {
                // Original v1 coupling (coherence -> cooperation)
    ...
    for seed in 0..seeds {
        ...
        for round in 0..rounds {
            ...
            let acts: Vec<Action> = agents.iter_mut()
                .map(|a| a.act(&markers, round)).collect();
...
fn main() {
    println!("\n{}", "=".repeat(70));
    println!("STRATEGY LAYER v2 - BASELINE-FIRST VALIDATION");
    println!("Primary: ON score > Baseline score");
    println!("Target: 2/3 games ON > Baseline");
    println!("{}", "=".repeat(70));
