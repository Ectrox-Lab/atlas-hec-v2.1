...
**Version**: v2.1  
**Date**: 2026-03-15  
**Status**: ACTIVE (Mainline Hard Rules Enforced)  
**Next Review**: After L4-v2 128-Seed Validation
...
## 主线硬规则（Mainline Hard Rules）

以下四条规则是 Atlas-HEC 主线的不可违反纪律，优先级高于任何阶段性成果或局部最优。

### Rule-1: 无终局锁定（No Final Lock Before Final Brain）

**在真正最后的大脑结构被证明之前，不存在 FINAL / NO REOPEN / 永久锁定。**

任何当前最优配置都只允许记为：
- **当前默认**（current default）
- **当前最佳候选**（current best candidate）
- **当前阶段生产起点**（current production starting point）
- **下一轮父代材料**（parent material for next round）

**禁止事项**：
- ❌ 把局部最优写成终局结论
- ❌ 使用 FINAL / NO REOPEN / PERMANENT / TERMINAL BASELINE 等词汇描述任何候选
- ❌ 将当前最佳候选视为停止演化的理由

**状态文件命名规范**：
- ✅ 使用 `STATUS_CURRENT_DEFAULT.lock`
- ✅ 使用 `STATUS_STAGE_DEFAULT.lock`
- ✅ 使用 `STATUS_TASK2_DEFAULT.lock`
- ❌ 禁止使用 `FINAL`、`NO_REOPEN`、`PERMANENT`

### Rule-2: 128 种子恒常（128-Seed Constant Exploration）

**在真正最后的大脑出现之前，每一轮核心实验必须维持 128 seeds 预算。**

执行流程固定为：
1. 从当前最佳候选集中选出精英集（elite set）
2. 对精英集做**组合**（recombine）
3. 对组合结果做**变形 / 扰动 / 重采样**（mutate / perturb / resample）
4. **重新扩展回 128 seeds**
5. 再进入筛选 / 验证 / 继承

**目的**：防止过早收敛、虚假终局感、单一路线坍塌。

**最小验证标准**：每轮实验登记必须包含以下字段：
```yaml
parent_candidates: [id_001, id_002, ...]  # 父代来源
reseed_count: 128                         # 必须 = 128
variation_ops: [recombine, mutate, resample]  # 变形操作列表
```

### Rule-3: 当前最佳的唯一合法用途

**当前最佳候选只能用于以下三种用途，任何其他用途均属违规。**

| 合法用途 | 说明 |
|---------|------|
| **当前默认基线** | 当前阶段可直接使用的起点配置 |
| **下一轮父代来源** | 提取其有效 contract / route / family / mechanism 结构 |
| **可继承材料** | 作为组合 / 变形 / 重采样的起点 |
...
**Family B v4.1.1 的正确表述示例**：
```yaml
Family_B_v4.1.1:
  status: "current default baseline for Task-2"
  role: "parent candidate set for next 128-seed expansion"
  scope: "Task-2 only"
  interpretation:
    - "usable now"
    - "preferred current baseline"
    - "not terminal"
    - "must re-enter recombination / mutation / reseeding to 128"
```

### Rule-4: 所有"最佳候选"必须回答 4 个问题

**任何被登记为当前最佳的候选，必须附带回答以下 4 个问题：**

| 问题 | 必须说明的内容 |
|------|---------------|
| **下一轮如何继续组合？** | 哪些 contract / mechanism 可被重组 |
| **下一轮如何继续变形？** | 允许的 mutation / perturbation 范围 |
| **如何重新扩展回 128 个种子？** | 重采样策略与多样性保证机制 |
| **是否正在导致过早收缩？** | 多样性指标监控与收缩风险评估 |

**不回答这 4 个问题的候选登记视为无效。**

---

### Rule-H1: 1小时反馈约束（1-Hour Feedback Constraint）

**任何单次实现或实验步骤必须在1小时现实时间内产生可观测反馈。**

**允许协议**:
1. 运行1小时探针
2. 检查该小时内是否出现正向信号
3. 只有存在正向信号，才能申请追加1小时
4. 否则停止，记录负结果，重新设计

**禁止事项**:
- ❌ 任何需要超过1小时才产生首次反馈的实验步骤
- ❌ 没有中间go/no-go检查点的长时运行
- ❌ 在Hour-1证据出现前预承诺大规模运行
- ❌ "理论上会好，但要几小时后才显现"
...
Hour-1 Probe
    ↓ (positive signal)
Hour-2 Extension (re-apply)
    ↓ (second positive)
Hour-3+ (re-apply each hour)
```

**违规惩罚**: 提议超过1小时实验 = 自动降级为Observer角色
...
### 14.2 实验结构（修正：符合 128-Seed 规则）

**⚠️ 根据主线硬规则 Rule-2，L4-v2 必须维持 128 种子恒常。**

修正后的实验结构：

- **Step 1 - Elite Selection**: 从 L4-v1 Round B 结果中选出精英候选（approve 候选中的 top 20%）
- **Step 2 - Recombination**: 对精英候选做 contract / mechanism / route 级别重组
- **Step 3 - Mutation & Perturbation**: 
  - Mechanism-level mutation（路由模式微调）
  - Anti-leakage constraint perturbation（带约束的扰动）
- **Step 4 - Reseed to 128**: 重新扩展回 **128 seeds**（原 30 → 现 128）
- **Step 5 - Split for Evaluation**:
  - **Round A-v2**: 128 种子中随机抽取 43 个，无 inheritance（control）
  - **Round B-v2**: 128 种子中随机抽取 43 个，使用 package v2 + anti-leakage
  - **Ablation-v2**: 128 种子中随机抽取 42 个，package loaded 但 bias=0.0, anti-leakage=0.0
- **Evaluation**: Bridge → Mainline（same thresholds）

**128-Seed 合规检查点**：
```yaml
l4v2_experiment:
  parent_candidates: 16                    # 从 L4-v1 精英集选出
  recombination_ops: [crossover, blend]    # 组合操作
  mutation_ops: [mechanism_mut, route_perturb]  # 变形操作
  resample_target: 128                     # 必须 = 128
  eval_sample_split: [43, 43, 42]          # 从 128 中抽取用于评估
  ```
...
**批准**: Atlas-HEC Research Committee  
**生效**: 2026-03-14  
**版本**: v2.1 (主线硬规则生效：Rule-1 无终局锁定 / Rule-2 128种子恒常 / Rule-3 当前最佳用途 / Rule-4 四问必须)
